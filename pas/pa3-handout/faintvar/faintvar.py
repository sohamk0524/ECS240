...
class FaintVariables(DataFlowAnalysis):
    ...
    def transfer_function(self, basic_block_id: int, dataflow_values: set) -> set:
        ...
        if stmt == None:
            ...
            if stmt[1] == None:
                #Constant assignment
                new_dataflow_values = dataflow_values.union({stmt[0]})
            else:
                #Variable Assigment
                if stmt[0] in dataflow_values:
                    gen = set(stmt[1])
                    new_dataflow_values = dataflow_values.union(gen)
                else:
                    kill = set(stmt[1])
                    new_dataflow_values = dataflow_values - kill

                    if stmt[0] not in stmt[1]:
                        #If lhs not on rhs, then lhs is faint as the IN of this basic block
                        new_dataflow_values = new_dataflow_values.union({stmt[0]})

            return new_dataflow_values

def faint_variable_info(num_vars: int):
    all_vars = [i for i in range(1, num_vars + 1, 1)]
    return {"all_vars": set(all_vars)}

def dump_faint_variables_info(output_path: str, faint_variables: FaintVariables):
    with open(output_path, "w") as file_obj:
        for basic_block_id in faint_variables.basic_blocks.keys():
            dataflow_values = faint_variables.basic_blocks_dataflow_values[basic_block_id][0]
            file_obj.write(f"fvin {basic_block_id}")
...
def main():
    ...
    faint_variables = FaintVariables(basic_blocks, cfg, rev_cfg, entry, exit, extra_info)
